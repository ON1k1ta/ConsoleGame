...
public class Object_4 {
    private int[][] M_cell = new int[4][2];                             //массив расположения объекта на поле содержит координаты объекта
    Field field = new Field();
    private int hit_counter = 0;                                        //создаем колличество попаданий по объекту

    public void CreateObject_4() {                                      //создание объекта на поле

        int[][] Lp = new int[][]{{-1, -1}, {-1, -1}, {-4, -1}, {-1, -4}};   //массив содержащий крайние координаты поля в которое будет помещен объект
        int[][] Rp = new int[][]{{4, 1}, {1, 4}, {1, 1}, {1, 1}};
        int Lpx, Lpy, Rpx, Rpy;                                             //переменые крайних точек этого поля

        loop:for(int count = 0; count < 18; count++) {                      //цикл повторений в случае неудачи создания объекта
            ...
            rotation:for(int j = random_place; j < 4; j++) {                //поворот корабля на 90 градусов относительно 1 клетки, если при первом построении в поле был другой объект
                        ...
                        one:switch (field.get(ix, iy)) {                    //получаем значение координат на основном поле игры
                            case -2:
                                switch (j) {                                //смотрим в какую сторону повернут объект в данный момент
                                    case 0:
                                        if((ix == unit_x+3) && (iy == unit_y)) break;   //проверяем за пределы поля вышла последняя клетка объекта, если да
                                        break one;                                      //поворачиваем объект
                                    case 1:
                                        if((ix == unit_x) && (iy == unit_y+3)) break;
                                        break one;
                                    case 2:
                                        if((ix == unit_x-3) && (iy == unit_y)) break;
                                        break one;
                                    case 3:
                                        if((ix == unit_x) && (iy == unit_y-3)) break;
                                        break one;
                                ...
                                continue rotation;
                            case 1:                                                     //если в пределах поля находится другой объект, то поворачиваем корабль
                                continue rotation;
                            case 0:
                                break;
                        }
                ...
                for(int i = 0; i<4; i++) {                                                //отмечаем координаты в массиве объекта и в поле
    ...
    //принимает координаты которые ввел пользователь
        for(int i = 0; i<4; i++) {                                          //проверка на попадание происходит по одной клетке за раз

            if((M_cell[i][0] == unit_x) && (M_cell[i][1] == unit_y)) {      //проверяет совпали ли координаты объекта и пользователя

                if(++hit_counter == 4) {                                    //проверяем на потопление корабля

                    for(int j = 0; j < 4; j++) {                            //помечаем область вокруг объекта (поле) как "промах"
                    ...
                    for(int i1 = 0; i1<4; i1++) {                           //помечаем на карте корабль как потопленный
